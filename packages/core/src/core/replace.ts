...
import { EVENTTYPES, HTTPTYPE, EMethods } from '@chengjb/common';
import { ReplaceHandler, voidFun } from '@chengjb/types';
...
function xhrReplace(): void {
  ...
  replaceAop(originalXhrProto, 'send', (originalSend: voidFun) => {
    return function (this: any, ...args: any[]): void {
      const { method, url } = this.chengjb_xhr;
      ...
      on(this, 'loadend', function (this: any) {
        ...
        this.chengjb_xhr.requestData = args[0];
        const eTime = getTimestamp();
        // 设置该接口的time，用户用户行为按时间排序
        this.chengjb_xhr.time = this.chengjb_xhr.sTime;
        this.chengjb_xhr.Status = status;
        if (['', 'json', 'text'].indexOf(responseType) !== -1) {
          ...
          if (options.handleHttpStatus && typeof options.handleHttpStatus == 'function') {
            this.chengjb_xhr.response = response && JSON.parse(response);
          }
        }
        // 接口的执行时长
        this.chengjb_xhr.elapsedTime = eTime - this.chengjb_xhr.sTime;
        // 执行之前注册的xhr回调函数
        notify(EVENTTYPES.XHR, this.chengjb_xhr);
      });
